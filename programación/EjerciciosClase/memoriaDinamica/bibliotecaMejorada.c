...
// Función para agregar un nuevo libro al catálogo. Se requiere un puntero a un arreglo de libros, el ID del nuevo libro, el título, autor, precio, género y cantidad en stock.
void addBook(Book** direction, int addId, const char* addTitle, const char* addAuthor, float addPrice, int addGender, int addQuantity) {
    ...
    // Si realloc fue exitoso, asignamos el puntero temporal a la dirección original del arreglo.
    *direction = temp;

    // Asignamos los datos del nuevo libro al nuevo espacio en el arreglo.
    ...
    (*direction)[booksQuantity].author[MAX_CHARS_AUTHOR - 1] = '\0';  // Aseguramos que el autor esté bien terminado con '\0'.
    ...
    // Ya no necesitamos devolver nada, ya que modificamos el puntero directamente.
}

// Función para agregar múltiples nuevos libros al catálogo. Se requiere el puntero a los libros y el número de nuevos libros a agregar.
void addNewBook(Book* data, int numNewBooks) {
    // Reasignamos memoria para los libros existentes más los nuevos libros. Calculamos el tamaño total necesario para todos los libros.
    Book* newData = (Book*)realloc(data, (booksQuantity + numNewBooks) * sizeof(Book));
    
    // Verificamos si realloc falló (si devuelve NULL, no pudo reasignar memoria).
    if (newData == NULL) {
        printf("Error: Couldn't reallocate memory.\n");
        free(data);  // Liberamos la memoria original.
        return;  // Terminamos la función si no se pudo reasignar memoria.
    }

    // Actualizamos el puntero de los libros con la nueva memoria.
    data = newData;

    // Indicamos que vamos a agregar nuevos libros.
    ...
    for (int i = 0; i < numNewBooks; i++) {
        ...
        // Pedimos el título del libro y lo almacenamos en el arreglo.
        printf("\tEnter the title: ");
        getchar();  // Limpiamos el buffer de entrada antes de usar fgets.
        ...
        // Pedimos el autor del libro y lo almacenamos en el arreglo.
        ...
        // Pedimos el precio del libro y lo almacenamos en el arreglo.
        printf("\tEnter the price: ");
        scanf("%f", &newData[booksQuantity + i].price);

        // Pedimos el género del libro, solicitando un número entre 0 y 4 que representará un género.
        printf("\tEnter the genre (0: Fiction, 1: Non fiction, 2: Poetry, 3: Theater, 4: Essay): ");
        scanf("%d", (int*)&newData[booksQuantity + i].typeGenre);

        // Pedimos la cantidad de stock disponible del nuevo libro y la almacenamos en el arreglo.
