...
//Creamos una función para confirmar si se debe realizar una funcionalidad o no
int checkInput(char input /*El valor (y/n) que ha introducido el usuario*/) { 
	if (input == 'y') {
		return 1;
	} else if (input == 'n') {
		return 0;
	} else {
		printf("Enter a valid argument.\n"); //Si no ha metido ninguno de los dos pues da error y devuelve un -1 para que se repita la función en la que se haya introducido un argumento inválido
		return -1;
	}
}
